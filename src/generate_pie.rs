...
pub async fn generate_pie(
    ...
    let (elapsed, stderr_output) = execute_with_streaming_output(&mut cmd, "generate-pie")?;
...
async fn is_generate_pie_available() -> bool {
    ...
    match Command::new("generate-pie")
        .arg("--help")
        .output()
    {
...
pub fn format_block_numbers(input: &str) -> Result<String> {
    ...
    if input.contains('-') {
        ...
        if parts.len() != 2 {
            return Err(anyhow::anyhow!("Invalid range format. Expected: start-end (e.g., \"100-110\")"));
        }
        ...
        if start > end {
            return Err(anyhow::anyhow!("Invalid range: start block ({}) must be less than or equal to end block ({})", start, end));
        }
