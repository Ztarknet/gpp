...
pub fn print_proof_public_data(proof: &CairoProof<Blake2sMerkleHasher>) {
    ...
    // Deserialize BootloaderOutput (first 3 felts)
    if public_output.len() < 3 {
        info!(
            "Warning: Public output too short to contain bootloader wrapper (length: {})",
            public_output.len()
        );
        info!("Raw public output: {:?}", public_output);
        return;
    }
    ...
    info!("");
    info!("--- BootloaderOutput ---");
    info!("  n_tasks:            {}", bootloader_output.n_tasks);
    info!(
        "  task_output_size:   {} felts",
        bootloader_output.task_output_size
    );
    info!(
        "  task_program_hash:  0x{:x}",
